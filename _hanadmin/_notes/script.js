	...
	$('#note-submit').live('click',function(e){
		...
		$.post('ajax/post.php',data,function(msg){
			...
			if(parseInt(msg))
			{
				...
				tmp.find('span.data').text(msg).end().css({'z-index':zIndex,top:10,left:50});
				tmp.appendTo($('#main'));
...
function memo_del(elements){
	elements.children(".close").mousedown(function(){
		if(confirm("정말로 삭제하시겠습니까?")){
			idval = parseInt($(this).parent().find('span.data').html());
			$.get('ajax/delete_memo.php',{
				id	: idval
			});
			elements.html("<span style='font-size:12px'>삭제되었습니다.</span>");
			setTimeout(function() { elements.fadeOut('slow'); }, 1000);
		}
	});
}