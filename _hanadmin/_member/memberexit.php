<?php
	$pagecode = "E00112";
  define("_administrators_",true);
  define("_g_board_include_",true);
  include "../../_core/_lib.php";

	/*-------------------------------------------------------------------------------------------------
	▶ 데이터베이스 연결 */	
	unset($db);
	$db = new MySQL;

	/*-------------------------------------------------------------------------------------------------
	▶ 게시판 정보 */	
	$Board['board_id'] = "member";
	$Board['board_name'] = "탈퇴회원";
	$Board['table_board'] = "G_Member";
	$Board['table_attach'] = "";
	$Board['page_limit'] = 20;
	$Board['page_block'] = 10;
	$Board['use_file'] = 10;
	$Board['use_reply'] = "N";

	$Board[use_comment] = "N";
	$Board[use_category] = "N";
	$Board[subject_length_main] = 30;
	$Board[subject_length_board] = 60;
	$Board[content_length] = 200;
	$Board[thumb_s_size] = 133;
	$Board[thumb_m_size] = 500;
	$Board[thumb_b_size] = 650;
	$Board[file_max_size] = "20M";
	$Board[file_check_type] = "deny";
	$Board[file_check_ext] = "php|phps|html|htm|exe|bat|sql";
	$Board[level_list] = 0;
	$Board[level_view] = 0;
	$Board[level_write] = 1;
	$Board[level_modify] = 1;
	$Board[level_reply] = 1;
	$Board[level_download] = 0;
	$Board[level_delete] = 1;
	$Board[level_comment] = 1;

	/*-------------------------------------------------------------------------------------------------
	▶ 변수 체크 */
	// 기본 변수
	$req['idx']        = Request('idx');
	$req['mid']        = Request('mid');
	$req['pagenumber'] = Request('pagenumber');
	$req['pagenumber'] = SetValue($req['pagenumber'],'digit', 1);

	// 검색변수
	$req['sn'] = Request('sn');
	$req['st'] = Request('st');	
	$req['sc'] = Request('sc');	
	$req['sp'] = Request('sp');	
	$req['se'] = Request('se');	
	$req['sa'] = Request('sa');	
	$req['ss'] = Request('ss');	
	$req['so'] = Request('so');	
	$req['sr'] = Request('sr');	

	$req['sdate'] = Request('sdate');
	$req['edate'] = Request('edate');

	// 정렬
	$req['orderby'] = Request('orderby');	
	$req['sort']    = Request('sort');	

  if(!$req['ss']) $req['ss'] = array("s1");
  if(!$req['so']) $req['so'] = array("s1","s2","s3");
  if(!$req['sr']) $req['sr'] = array("0","1","2","3","4","5");


	$Board['Link'] = $_SERVER['SCRIPT_NAME'];
	$href  = $Board['Link']."?";
	$href .= "pagenumber=".$req['pagenumber'];
	$href .= "&sn=".$req['sn'];
	$href .= "&st=".$req['st'];
	$href .= "&sc=".$req['sc'];
	$href .= "&sp=".$req['sp'];
	$href .= "&se=".$req['se'];
	$href .= "&sa=".$req['sa'];
  foreach($req['ss'] as $ssk => $ssv)	$href .= "&ss[]=".$ssv;
  foreach($req['so'] as $sok => $sov)	$href .= "&so[]=".$sov;
  foreach($req['sr'] as $srk => $srv)	$href .= "&sr[]=".$srv;

	$href .= "&sdate=".$req['sdate'];
	$href .= "&edate=".$req['edate'];
	$href .= "&orderby=".$req['orderby'];
	$href .= "&sort=".$req['sort'];

  $_Reason['A'] = "승인대기시간";
  $_Reason['B'] = "마음에 드는 이성을 만나지 못함";
  $_Reason['C'] = "개인 정보 유출 우려";
  $_Reason['D'] = "고객서비스 불만";
  $_Reason['E'] = "아이템/결제";
  $_Reason['F'] = "인터넷을 통한 만남의 어려움";
  $_Reason['G'] = "기타";

	/*-------------------------------------------------------------------------------------------------
	▶ 페이지 호출 */
	$req['at'] = Request('at');
	$include_page = null;
	switch($_GET['at']){
		case "view": 
			$include_page = "memberexit_view.php"; break;
		case "dataprocess": 
			$include_page = "memberexit_action.php"; break;
		case "excel": 
			$include_page = "memberexit_excel.php"; break;
		case "download": 
			download();		break;
		default:
			$include_page = "memberexit_list.php"; break;
	}

	if($include_page && file_exists($include_page)) include $include_page;


?>