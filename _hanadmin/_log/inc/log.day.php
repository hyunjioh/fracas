<?
  ...
  include "../index.php";
	...
	$req['sdate'] = (!$_GET['sdate'])?  date("Y-m") : substr($_GET['sdate'],0,7);
	$req['edate'] = (!$_GET['edate'])?  date("Y-m-d") : $_GET['edate'];

	$GraphName = $req['sdate']." 일별 방문자수";
  ...
  $req['sdatesp'] = explode("-",$req['sdate']);
  for($i=0; $i < 31; $i++){
    $j = $i+1;
    if($j < 10) $j = "0".$j;
    if(checkdate($req['sdatesp'][1],$j,$req['sdatesp'][0]) && !isset($list[$req['sdate']."-".$j])){
      $list[$req['sdate']."-".$j] = 0;
    }  
  }
	ksort($list);
