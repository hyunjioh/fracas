<?
  ...
  include "../index.php";
	...
	$Query = "SELECT min(uid) as uid, min(SUBSTRING_INDEX(page,'?',1)) as page, sid FROM `G__Log_History` WHERE  ".$WhereQuery." group by sid";
	//$Query = "SELECT min(uid) as uid, min(SUBSTRING_INDEX(page,'?',1)) as page, sid FROM `G__Log_History` WHERE  time LIKE '".$date."%' group by sid";
	...
	$log_First['list'] = $list;
	$log_First['list_total'] = $list_total;

	$log_First['graph'] = $graph;
	$log_First['graph_total'] = $graph_total;
	$log_First['max'] = ceil(($max+1)/10)*10;
	$total = $log_First['graph_total'];

	if($total > 0 ){
		$data_keys = implode("|",array_keys($log_First['graph']));
		$data_values = implode("|",array_values($log_First['graph']));

    $Count['total']  = $log_First['list_total'];
    $Count['top5']   = $log_First['graph_total'];
    $Count['etc']    = $log_First['list_total'] - $log_First['graph_total'];
    $Percent['top5'] = ($log_First['graph_total']/$log_First['list_total'])*100;
    $Percent['etc']  = (( $log_First['list_total'] - $log_First['graph_total'])/$log_First['list_total'])*100;
	}else{
		$data_keys = null;
		$data_values = null;
    echo "데이터가 없습니다.";
...
$(function () {
    ...
    $(document).ready(function() {
        ...
        var table = document.getElementById('firstpage_datatable'),
        options = {
            chart: {
              animation: false,
              height : 400,
              renderTo: 'firstpage_graph',
...
<table id="firstpage_datatable" style="display:none">
	<thead>
		<tr>
			<th></th>
			<th><?=$GraphName?></th>
...
<div id="firstpage_graph" class="graph_container"></div>
