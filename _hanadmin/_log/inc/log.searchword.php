<?
  ...
  include "../index.php";
	...
	$Query = "SELECT search,count(*) AS cnt FROM `G__Log_Ref` WHERE search is not null and search <> '' and ".$WhereQuery." group by search ORDER BY cnt DESC, name ASC";
	//$Query = "SELECT search,count(*) AS cnt FROM `G__Log_Ref` WHERE search is not null and search <> '' and time LIKE '".$date."%' group by search ORDER BY cnt DESC, name ASC";
	...
	if($cnt > 0){
		...
		foreach($DATA as $key => $value){
			...
			$list[$value['search']] = (int)$value['cnt'];
			if($i <= 5){
	...
	$log_Search['list'] = $list;
	$log_Search['list_total'] = $list_total;
	$log_Search['graph'] = $graph;
	$log_Search['graph_total'] = $graph_total;
	$log_Search['max'] = ceil(($max+1)/10)*10;

	$total = $log_Search['graph_total'];

	if($total > 0 ){
		$data_keys = implode("|",array_keys($log_Search['graph']));
		$data_values = implode("|",array_values($log_Search['graph']));

    $Count['total']  = $log_Search['list_total'];
    $Count['top5']   = $log_Search['graph_total'];
    $Count['etc']    = $log_Search['list_total'] - $log_Search['graph_total'];
    $Percent['top5'] = ($log_Search['graph_total']/$log_Search['list_total'])*100;
    $Percent['etc']  = (( $log_Search['list_total'] - $log_Search['graph_total'])/$log_Search['list_total'])*100;
	}else{
		$data_keys = null;
		$data_values = null;
    echo "데이터가 없습니다.";
...
$(function () {
    ...
    $(document).ready(function() {
        ...
        var table = document.getElementById('word_datatable'),
        options = {
            chart: {
              animation: false,
              height : 400,
...
<table id="word_datatable" style="display:none">
	<thead>
		<tr>
			<th></th>
			<th><?=$GraphName?></th>
...
<div id="word_graph" class="graph_container"></div>
