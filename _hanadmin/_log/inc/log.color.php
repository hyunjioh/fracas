<?
  ...
  include "../index.php";
	...
	$Query = "SELECT cd,count(*) AS cnt FROM `G__Log_UserInfo` WHERE  ".$WhereQuery." group by cd ORDER BY cnt DESC, cd DESC";
	//$Query = "SELECT cd,count(*) AS cnt FROM `G__Log_UserInfo` WHERE  time LIKE '".$date."%' group by cd ORDER BY cnt DESC, cd DESC";
	...
	if($cnt > 0){
		...
		foreach($DATA as $key => $value){
			...
			$list[$cfg['color'][$value['cd']]] = (int)$value['cnt'];
	...
	$log_Color['list'] = $list;
	$log_Color['list_total'] = $list_total;
	$log_Color['graph'] = $graph;
	$log_Color['graph_total'] = $graph_total;
	$log_Color['max'] = ceil(($max+1)/10)*10;

	$total = $log_Color['graph_total'];

	if($total > 0 ){
		$data_keys = implode("|",array_keys($log_Color['graph']));
		$data_values = implode("|",array_values($log_Color['graph']));

    $Count['total']  = $log_Color['list_total'];
    $Count['top5']   = $log_Color['graph_total'];
    $Count['etc']    = $log_Color['list_total'] - $log_Color['graph_total'];
    $Percent['top5'] = ($log_Color['graph_total']/$log_Color['list_total'])*100;
    $Percent['etc']  = (( $log_Color['list_total'] - $log_Color['graph_total'])/$log_Color['list_total'])*100;
	}else{
		$data_keys = null;
		$data_values = null;
    echo "데이터가 없습니다.";
...
$(function () {
    ...
    $(document).ready(function() {
        ...
        var table = document.getElementById('color_datatable'),
        options = {
            chart: {
              animation: false,
              height : 400,
              renderTo: 'color_graph',
...
<table id="color_datatable" style="display:none">
	<thead>
		<tr>
			<th></th>
			<th><?=$GraphName?></th>
...
<div id="color_graph" class="graph_container"></div>
