<?
  ...
  include "../index.php";
	...
	$Query = "SELECT ref, count(*) as cnt FROM `G__Log_Basic` WHERE ( ref <> 'bookmark' and ref <> '' ) and ".$WhereQuery." GROUP BY ref ORDER BY cnt DESC";
	//$Query = "SELECT ref, count(*) as cnt FROM `G__Log_Basic` WHERE ( ref <> 'bookmark' and ref <> '' ) and time LIKE '".$date."%' GROUP BY ref ORDER BY cnt DESC";
	...
	$log_Link['list'] = $list;
	$log_Link['list_total'] = $list_total;
	$log_Link['graph'] = $graph;
	$log_Link['graph_total'] = $graph_total;
	$log_Link['max'] = ceil(($max+1)/10)*10;

	$total = $log_Link['graph_total'];

	if($total > 0 ){
		$data_keys = implode("|",array_keys($log_Link['graph']));
		$data_values = implode("|",array_values($log_Link['graph']));

    $Count['total']  = $log_Link['list_total'];
    $Count['top5']   = $log_Link['graph_total'];
    $Count['etc']    = $log_Link['list_total'] - $log_Link['graph_total'];
    $Percent['top5'] = ($log_Link['graph_total']/$log_Link['list_total'])*100;
    $Percent['etc']  = (( $log_Link['list_total'] - $log_Link['graph_total'])/$log_Link['list_total'])*100;
	}else{
		$data_keys = null;
		$data_values = null;
    echo "데이터가 없습니다.";
...
$(function () {
    ...
    $(document).ready(function() {
                          ...
                          if(j > 0 ){
                            ...
                            dataname = this.innerHTML;
														if(dataname.length > 50){
															dataname = dataname.substring(0,50)+"...";
														}
                          }
        ...
        var table = document.getElementById('link_datatable'),
        options = {
            chart: {
              animation: false,
              height : 400,
                ...
                pie: {
                    ...
                    dataLabels: {
                        ...
                        formatter: function() {
                            ...
                            return '<b>'+ this.point.name.substring(0,30) +'...</b>: '+ parseInt(this.percentage) +' %';
													}else{
...
<table id="link_datatable" style="display:none">
	<thead>
		<tr>
			<th></th>
			<th><?=$GraphName?></th>
...
<div id="link_graph" class="graph_container"></div>
<div class="pageContent">
	<ul class="accordion">
      ...
      ?>
				<li>
					<dl>
						<dt class="number"><a href="<?=$key?>" target="_blank"><? if($i  < 6) echo "<strong>";?><?=$i?>. <?=(strlen($key) > 70)? substr($key,0,70)."..":$key; ?><? if($i  < 6) echo "</strong>";?></a> <? //ip2flag($key)?></dt>
