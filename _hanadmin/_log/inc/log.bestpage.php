<?
  ...
  include "../index.php";
	...
	$Query = "SELECT count(*) AS cnt, SUBSTRING_INDEX(page,'?',1) as page FROM `G__Log_History` WHERE  ".$WhereQuery." group by SUBSTRING_INDEX(page,'?',1)  ORDER BY cnt DESC, page ASC";
	//$Query = "SELECT page,count(*) AS cnt, SUBSTRING_INDEX(page,'?',1) FROM `G__Log_History` WHERE  time LIKE '".$date."%' group by SUBSTRING_INDEX(page,'?',1)  ORDER BY cnt DESC, page ASC";
  ...
  $max = 0;
	...
	$log_Popular['list'] = $list;
	$log_Popular['list_total'] = $list_total;
	$log_Popular['graph'] = $graph;
	$log_Popular['graph_total'] = $graph_total;
	$log_Popular['max'] = ceil(($max+1)/10)*10;
	$total = $log_Popular['graph_total'];

	if($total > 0 ){
		$data_keys = implode("|",array_keys($log_Popular['graph']));
		$data_values = implode("|",array_values($log_Popular['graph']));

    $Count['total']  = $log_Popular['list_total'];
    $Count['top5']   = $log_Popular['graph_total'];
    $Count['etc']    = $log_Popular['list_total'] - $log_Popular['graph_total'];
    $Percent['top5'] = ($log_Popular['graph_total']/$log_Popular['list_total'])*100;
    $Percent['etc']  = (( $log_Popular['list_total'] - $log_Popular['graph_total'])/$log_Popular['list_total'])*100;
	}else{
		$data_keys = null;
		$data_values = null;
    echo "데이터가 없습니다.";
...
$(function () {
    ...
    $(document).ready(function() {
        ...
        var table = document.getElementById('bestpage_datatable'),
        options = {
            chart: {
              animation: false,
              height : 400,
              renderTo: 'bestpage_graph',
...
<table id="bestpage_datatable" style="display:none">
	<thead>
		<tr>
			<th></th>
			<th><?=$GraphName?></th>
...
<div id="bestpage_graph" class="graph_container"></div>
