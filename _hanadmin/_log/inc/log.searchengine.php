...
//	$Query = "SELECT ifnull(name,'직접방문') as name, count(*) AS cnt FROM `G__Log_Ref` WHERE name <> 'bookmark' and ".$WhereQuery." group by name ORDER BY cnt DESC, name ASC";
	$Query = "SELECT ifnull(name,'직접방문') as name, count(*) AS cnt FROM `G__Log_Ref` WHERE  ".$WhereQuery." group by name ORDER BY cnt DESC, name ASC";
	...
	$log_Engine['list'] = $list;
	$log_Engine['list_total'] = $list_total;
	$log_Engine['graph'] = $graph;
	$log_Engine['graph_total'] = $graph_total;
	$log_Engine['max'] = ceil(($max+1)/10)*10;
	$total = $log_Engine['graph_total'];

	if($total > 0 ){
		$data_keys = implode("|",array_keys($log_Engine['graph']));
		$data_values = implode("|",array_values($log_Engine['graph']));


    $Count['total']  = $log_Engine['list_total'];
    $Count['top5']   = $log_Engine['graph_total'];
    $Count['etc']    = $log_Engine['list_total'] - $log_Engine['graph_total'];
    $Percent['top5'] = ($log_Engine['graph_total']/$log_Engine['list_total'])*100;
    $Percent['etc']  = (( $log_Engine['list_total'] - $log_Engine['graph_total'])/$log_Engine['list_total'])*100;
	}else{
		$data_keys = null;
		$data_values = null;
    echo "데이터가 없습니다.";
...
$(function () {
    ...
    $(document).ready(function() {
        ...
        var table = document.getElementById('engine_datatable'),
        options = {
            chart: {
              animation: false,
              height : 400,
              renderTo: 'engine_graph',
...
<table id="engine_datatable" style="display:none">
	<thead>
		<tr>
			<th></th>
			<th><?=$GraphName?></th>
...
<div id="engine_graph" class="graph_container"></div>
