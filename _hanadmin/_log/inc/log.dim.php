<?
  ...
  include "../index.php";
	...
	$Query = "SELECT dim,count(*) AS cnt FROM `G__Log_UserInfo` WHERE  ".$WhereQuery." group by dim ORDER BY cnt DESC, dim DESC";
	//$Query = "SELECT dim,count(*) AS cnt FROM `G__Log_UserInfo` WHERE  time LIKE '".$date."%' group by dim ORDER BY cnt DESC, dim DESC";
	...
	$log_Dim['list'] = $list;
	$log_Dim['list_total'] = $list_total;
	$log_Dim['graph'] = $graph;
	$log_Dim['graph_total'] = $graph_total;
	$log_Dim['max'] = ceil(($max+1)/10)*10;
	$total = $log_Dim['graph_total'];

	if($total > 0 ){
		$data_keys = implode("|",array_keys($log_Dim['graph']));
		$data_values = implode("|",array_values($log_Dim['graph']));

    $Count['total']  = $log_Dim['list_total'];
    $Count['top5']   = $log_Dim['graph_total'];
    $Count['etc']    = $log_Dim['list_total'] - $log_Dim['graph_total'];
    $Percent['top5'] = ($log_Dim['graph_total']/$log_Dim['list_total'])*100;
    $Percent['etc']  = (( $log_Dim['list_total'] - $log_Dim['graph_total'])/$log_Dim['list_total'])*100;
	}else{
		$data_keys = null;
		$data_values = null;
    echo "데이터가 없습니다.";
...
$(function () {
    ...
    $(document).ready(function() {
      ...
      var table = document.getElementById('dim_datatable'),
      options = {
          chart: {
            animation: false,
            height : 400,
...
<table id="dim_datatable" style="display:none">
	<thead>
		<tr>
			<th></th>
			<th><?=$GraphName?></th>
...
<div id="dim_graph" class="graph_container"></div>
