<?
  ...
  $Query = "select avg(c) as cnt, day from ( select TIMESTAMPDIFF(second, min(time), max(time)) as c, (select cast(time as date) as day from G__Log_Basic Where G__Log_Basic.sid = G__Log_History.sid) as day
 from G__Log_History Where sid in (SELECT sid FROM `G__Log_Basic`  Where ".$WhereQuery." ) group by sid having ( count( sid ) > 1 ) ) as ddd group by day 
";
...
<table width="100%" cellpadding="0" cellspacing="0" class="Info">
  ...
  <?
    ...
    foreach($log_Date['list'] as $key => $value){
      ...
      $hourvalue = floor($value/3600);
      if($hourvalue > 0){
        $revalue = $hourvalue."h";
        $numvalue = $value - $hourvalue*3600  ;

        if($numvalue > 60){
          $minutevalue = floor($numvalue/60);
          $revalue .= " ".$minutevalue."m ".($numvalue - $minutevalue*60)."s";
        }else{
          $revalue .= " 0m ".$numvalue."s";
        }
      }else{
        $revalue = "";
        $numvalue = $value - $hourvalue*3600  ;
  ...
  <tr height='22'  bgcolor="<?=$cellcolor?>">
    ...
    <td align="right"style="padding-right:5px"><span class="number-small"><?=number_format($value)?> sec</span> <span class="number-normal">(<?=$revalue?>)</span></td>
  </tr>
  <?
    $i++;
    if($i < $count)	{
  ...
  <?
    $value = round(array_sum($list)/ $count);
    $hourvalue = floor($value/3600);
    if($hourvalue > 0){
      $revalue = $hourvalue."h";
      $numvalue = $value - $hourvalue*3600  ;
  ...
  ?>
  <tr  bgcolor="#efefef">
    <td align='left' style="padding-left:5px; border-left:1px solid #cccccc"><span class="number-big">Average</span></td>
    <td align="right" style="padding-right:5px"><span class="number-middle"><?=$value?> sec</span> <span class="number-middle">(<?=$revalue?>)<span></td>
  </tr>
</table>
<?
}
