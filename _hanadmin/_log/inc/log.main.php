<?
  ...
  include "../index.php";
	...
	$req['sdate'] = (!$_GET['sdate'])?  date("Y") : substr($_GET['sdate'],0,4);
	$req['edate'] = (!$_GET['edate'])?  date("Y-m-d") : $_GET['edate'];
	$GraphSubTitle = ($req['sdate'] == $req['edate']) ? $req['sdate']: $req['sdate']." ~ ".$req['edate'];


	$GraphName = $req['sdate']."년 월별 방문자";
	$WhereQuery = " time like '".$req['sdate']."%'";
	$Query = "SELECT count(*) as cnt, cast(time as char(7) ) as day FROM `G__Log_Basic` WHERE ".$WhereQuery." GROUP BY month(time)";
