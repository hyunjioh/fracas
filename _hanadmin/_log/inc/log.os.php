<?
  ...
  include "../index.php";
	...
	$Query = "SELECT os,count(*) AS cnt FROM `G__Log_UserInfo` WHERE ".$WhereQuery." group by os ORDER BY cnt DESC, os ASC";
	//$Query = "SELECT os,count(*) AS cnt FROM `G__Log_UserInfo` WHERE time LIKE '".$date."%' group by os ORDER BY cnt DESC, os ASC";
	...
	$log_OS['list'] = $list;
	$log_OS['list_total'] = $list_total;
	$log_OS['graph'] = $graph;
	$log_OS['graph_total'] = $graph_total;
	$log_OS['max'] = ceil(($max+1)/10)*10;
	$total = $log_OS['graph_total'];

	if($total > 0 ){
		$data_keys = implode("|",array_keys($log_OS['graph']));
		$data_values = implode("|",array_values($log_OS['graph']));

    $Count['total']  = $log_OS['list_total'];
    $Count['top5']   = $log_OS['graph_total'];
    $Count['etc']    = $log_OS['list_total'] - $log_OS['graph_total'];
    $Percent['top5'] = ($log_OS['graph_total']/$log_OS['list_total'])*100;
    $Percent['etc']  = (( $log_OS['list_total'] - $log_OS['graph_total'])/$log_OS['list_total'])*100;
	}else{
		$data_keys = null;
		$data_values = null;
    echo "데이터가 없습니다.";
...
<table id="os_datatable" style="display:none">
	<thead>
		<tr>
			<th></th>
			<th><?=$GraphName?></th>
...
<div id="os_graph" class="graph_container"></div>
