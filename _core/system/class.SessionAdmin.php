...
class session { 
	...
	function read($sessID) { 
		// fetch session-data 
		$res = @mysql_query("SELECT sdata AS d FROM G_SESSION_ADMIN 
												WHERE session_id = '$sessID' 
												AND session_expires > ".time(),$this->dbHandle); 
		// return data or an empty string at failure 
    if(!is_resource($res)){
      $table = "G_SESSION_ADMIN";
      $exists = mysql_query('SHOW TABLES FROM `'.db_name.'` LIKE \''.$table.'\'', $this->dbHandle);

      if(mysql_num_rows($exists) < 1){
        $Query = "
          CREATE TABLE IF NOT EXISTS `G_SESSION_ADMIN` (
	...
	function write($sessID,$sessData) { 
		...
		$res = mysql_query("SELECT * FROM G_SESSION_ADMIN 
												WHERE session_id = '$sessID'",$this->dbHandle); 
		// if yes, 
		if(mysql_num_rows($res)) { 
			// ...update session-data 
			mysql_query("UPDATE G_SESSION_ADMIN 
		...
		// if no session-data was found, 
			...
			// create a new row 
			$res = @mysql_query("INSERT INTO G_SESSION_ADMIN ( 
									 session_id, 
									 session_expires, 
									 sdata) 
									 VALUES( 
	...
	function destroy($sessID) { 
		// delete session-data 
		mysql_query("DELETE FROM G_SESSION_ADMIN WHERE session_id = '$sessID'",$this->dbHandle); 
	...
	function gc($sessMaxLifeTime) { 
		// delete old sessions 
		mysql_query("DELETE FROM G_SESSION_ADMIN WHERE session_expires < ".time(),$this->dbHandle); 
