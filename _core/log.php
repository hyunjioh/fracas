...
###################################################################################################
	...
	$FieldArray = array(
		...
		'ip' 	   => $value['ip'],
		'sid'	   => $value['sid'],
		'mid'	   => $_SESSION['_MEMBER_']['ID'],
		'time'	 => $value['now'] ,
	);
	/* IP trace */
	$TRACE = $db -> SelectOne("SELECT Country, Region FROM G__Log_Basic WHERE ip='".$value['ip']."'");
	if($TRACE){
		// 국가
		$value['Country'] = $TRACE['Country'];
		// 지역
		$value['Region']  = $TRACE['Region'];		
	}else{
		...
		// 국가
		$value['Country'] = $arrXml[result][countrycode];
		// 지역
		$value['Region'] = $arrXml[result][region];
		// ISP
		$value['ISP'] = $arrXml[result][isp];
		// 위도
		$value['LAT'] = $arrXml[result][latitude];
		// 경도
		$value['LNG'] = $arrXml[result][longitude];
	}
	...
	$FieldArray = array_merge($FieldArray, $FieldArrayAdd);
